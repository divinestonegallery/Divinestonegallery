...
import { getDraftPagePreview } from "@/server/backend-api-client";
...
export default async function DraftPreviewPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params; const page = await getDraftPagePreview<PreviewPage>(decodeURIComponent(id)); if (!page) notFound();
