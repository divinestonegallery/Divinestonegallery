...
export default function PrivacyPage() {
  ...
  return (
    <LegalPage title="Privacy" eyebrow="Your information" intro="How Divine Stone Gallery collects, uses and protects information when you browse, create an account, place an order or commission a sacred work.">
      ...
      <section><h2 className="font-display">Service providers</h2><p>Only when the relevant feature is configured, information may be processed by providers that help operate the service: Clerk for authentication; Cloudflare for hosting, database and private media storage; Razorpay for online payments; Shiprocket and selected couriers for delivery rates and fulfilment; Resend for email; MSG91 for SMS; and Meta for WhatsApp. Payment-card or UPI credentials are entered into Razorpay&apos;s payment experience and are not stored in the Divine Stone Gallery database. Each provider also applies its own terms and privacy practices.</p></section>
      ...
      <section className={styles.contactBox}><h2 className="font-display">Privacy contact</h2><p>{business.grievanceOfficerName ? `${business.grievanceOfficerName}, grievance officer. ` : ""}Email <a href={`mailto:${business.grievanceEmail || "divinestonegallery@gmail.com"}`}>{business.grievanceEmail || "divinestonegallery@gmail.com"}</a>, use the <Link href="/contact">contact page</Link>, or call {business.grievancePhone || "+91 63768 71065"}. {business.customerCareHours ? `Customer-care hours: ${business.customerCareHours}. ` : ""}We may verify your identity before acting on a request concerning private account or order information.</p></section>
    </LegalPage>
  );
}
