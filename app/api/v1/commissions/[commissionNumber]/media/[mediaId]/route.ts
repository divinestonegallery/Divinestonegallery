...
import { authorizeCustomer, authorizeStaff } from "@/auth/authorization";
import { getDb } from "@/db";
import { commissionMedia, commissionMilestones, customCommissions, mediaAssets, milestoneMedia } from "@/db/schema";
...
export async function GET(request: Request, { params }: { params: Promise<{ commissionNumber: string; mediaId: string }> }) {
  ...
  const [reference] = await db.select({ r2Key: mediaAssets.r2Key, contentType: mediaAssets.contentType }).from(mediaAssets)
  ...
  const [milestone] = reference ? [] : await db.select({ r2Key: mediaAssets.r2Key, contentType: mediaAssets.contentType }).from(mediaAssets)
