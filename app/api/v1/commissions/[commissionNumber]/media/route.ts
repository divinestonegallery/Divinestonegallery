...
import { attachCommissionReference } from "@/commissions/repository";
...
export async function POST(request: Request, { params }: { params: Promise<{ commissionNumber: string }> }) {
  ...
  if (!stored) return Response.json({ error: { code: "INVALID_IMAGE", message: "Use a genuine JPEG, PNG or WebP image up to 12 MB." } }, { status: 400 });
  try {
