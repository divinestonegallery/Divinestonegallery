...
- Nine existing catalogue products, categories, deities, variants and image metadata are seeded into D1.
- `/shop` and product pages read the D1 catalogue, with the original local catalogue as a read-only development/degraded fallback.
...
- Staff can upload validated JPEG, PNG and WebP product photos up to 12 MB to R2.
...
Seed images continue to use their existing `/public/catalog` paths. New staff uploads are signature-checked, assigned server-generated object keys and stored in R2. D1 stores the product relationship, checksum, content type, byte size, alt text and processing state. R2 images are served through a public route only when attached to an active product.
