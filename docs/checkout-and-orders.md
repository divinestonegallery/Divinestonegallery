...
Step 6 added the secure transaction boundary. Step 7 now connects live Shiprocket surface rates and short-lived D1 shipping quotes. The application still deliberately refuses to invent shipping prices or silently choose an online payment gateway.
...
- concurrent duplicates are stopped by a unique D1 index;
- failed batches do not leave an order, payment or stock mutation behind.

A valid order also requires an active, unexpired shipping quote owned by the customer. The quote must match destination postcode, cart fingerprint, subtotal, GST, chargeable weight and final total. The quote is consumed within the same D1 batch as:
...
Every displayed option is stored in D1 for 30 minutes with its cart fingerprint, payment method, rate snapshot and per-package details. Selecting it does not reserve stock. The order transaction revalidates it before consuming it.
