...
CREATE TABLE `__new_product_variants` (
	...
	CONSTRAINT "product_variants_package_dimensions_positive" CHECK(("package_length_mm" is null or "package_length_mm" > 0) and ("package_width_mm" is null or "package_width_mm" > 0) and ("package_height_mm" is null or "package_height_mm" > 0)),
...
INSERT INTO `__new_product_variants`("id", "product_id", "sku", "name", "material", "finish", "height_mm", "width_mm", "depth_mm", "weight_grams", "package_length_mm", "package_width_mm", "package_height_mm", "price_paise", "gst_rate_bps", "inventory_kind", "stock_quantity", "cod_eligible", "shipping_class", "is_active", "created_at", "updated_at") SELECT "id", "product_id", "sku", "name", "material", "finish", "height_mm", "width_mm", "depth_mm", "weight_grams", NULL, NULL, NULL, "price_paise", "gst_rate_bps", "inventory_kind", "stock_quantity", "cod_eligible", "shipping_class", "is_active", "created_at", "updated_at" FROM `product_variants`;--> statement-breakpoint
